...
{
    /**
     * Tool única para todo lo relacionado con rutas: listar, recomendar, buscar,
     * ver detalle de la ruta seleccionada, revisar alertas y buscar puntos de interés.
     */
    public function routes(Request $request): JsonResponse
    {
        $payload = $request->validate([
            'intent' => ['nullable', 'string', 'in:list,recommend,detail,alerts,search,pois'],
            ...
            'poi_category' => ['nullable', 'string', 'max:80'],
        ...
        if ($intent === 'pois') {
            $pois = $this->searchPoiCards($payload, $latitude, $longitude, $limit, $route);
        ...
        $routes = $this->searchRouteContexts($payload, $latitude, $longitude, $limit);

        if ($routes === [] && isset($payload['query'])) {
            $fallbackPayload = $payload;
            unset($fallbackPayload['query']);

            $routes = $this->searchRouteContexts($fallbackPayload, $latitude, $longitude, $limit);
        }
     ...
     */
    private function searchRouteContexts(array $payload, ?float $latitude, ?float $longitude, int $limit): array
    {
        return $this->activeRoutesQuery()
    ...
    /**
     * @return array{average: float|null, total: int, reviews: array<int, array<string, mixed>>}
     */
    private function ratingsSummary(CyclingRoute $route): array
    {
        $approved = $route->ratings;
        $total = $approved->count();
        $average = $total > 0 ? round((float) $approved->avg('rating'), 1) : null;

        $reviews = $approved
            ->sortByDesc('rated_at')
            ->take(3)
            ->map(function (RouteRating $rating): array {
     ...
     * @return array<string, mixed>
     */
    private function buildPoiContext(PointOfInterest $poi, ?float $distanceMeters, ?CyclingRoute $route): array
    {
    ...
    private function searchPoiCards(array $payload, ?float $latitude, ?float $longitude, int $limit, ?CyclingRoute $route): array
    {
        return PointOfInterest::query()
            ...
            ->when(isset($payload['poi_category']), fn (Builder $query) => $query->whereHas('category', fn (Builder $categoryQuery) => $categoryQuery->where('name', 'like', '%'.$payload['poi_category'].'%')))
            ->when(isset($payload['query']), fn (Builder $query) => $query->where(function (Builder $textQuery) use ($payload): void {
                $textQuery->where('name', 'like', '%'.$payload['query'].'%')
                    ->orWhere('description', 'like', '%'.$payload['query'].'%')
            ...
            ->map(function (PointOfInterest $poi) use ($latitude, $longitude, $route): array {
                ...
                return [
                    ...$this->buildPoiContext($poi, $distanceMeters, $route),
                    '_sort_distance_m' => $distanceMeters,
                ];
            })
     ...
     * @return array<string, mixed>
     */
    private function buildAlertContext(Incident $incident): array
    {
