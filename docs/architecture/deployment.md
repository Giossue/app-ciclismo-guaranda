...
- El health check HTTP es `GET /up`: responde `200 {"status":"up"}` sin depender de Inertia SSR ni de diagnósticos de terceros. La imagen declara este chequeo con periodo inicial y Nginx espera a que PHP-FPM acepte conexiones antes de arrancar; Dokploy debe respetar el estado `healthy` antes de dirigir tráfico durante un despliegue.
